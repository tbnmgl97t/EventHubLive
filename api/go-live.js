 ...
 * Body: { id }
 *
 * Transitions a 24/7 channel from preview → live.
 * Only valid for stream_type = '24/7' channels in preview state.
 */
export default async function handler(req, res) {
  ...
  const url = `https://api.jwplayer.com/v2/sites/${jw.siteId}/live/broadcast/streams/${id}/go_live/`
  console.log('[go-live] PUT', url)
  ...
  try {
    ...
    console.log('[go-live] JW response', r.status, text.slice(0, 300))
