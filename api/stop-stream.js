 ...
 * Body: { id }
 *
 * Stops a live 24/7 broadcast stream, returning it to idle state.
 * Only valid for stream_type = '24/7' channels in active/streaming state.
 */
export default async function handler(req, res) {
  ...
  const url = `https://api.jwplayer.com/v2/sites/${jw.siteId}/live/broadcast/streams/${id}/stop/`
  console.log('[stop-stream] PUT', url)
  ...
  try {
    ...
    console.log('[stop-stream] JW response', r.status, text.slice(0, 300))
